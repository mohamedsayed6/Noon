...
export class CartComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem("currentUser")) {
      this._cartService.getCartItems().subscribe((prod) => (this.LocalStorageProducts = prod));
    } else {
  ...
  Remove(ProductID: number) {
    if (localStorage.getItem("currentUser")) {
      this._cartService.removeFromCart(ProductID).subscribe((result) => {
        setTimeout(() => {
          window.location.reload();
          console.log("asdasffsfsafugdsfgdggf");
        }, 5000);
      });
    } else {
